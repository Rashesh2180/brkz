...
const ToolsConsumbles = () => {
  const { loading, error, data } = useFetch("http://localhost:1337/api/products"); 
  const location = useLocation()

  const cementRouter = location.pathname === "/buymaterial/tools-consumables"
  ...
  const filteredData = data?.data?.filter(product =>
    categoriesToShow.includes(product.product_category)
  );
  ...
  return (
    ...
    <>
    {
      ...
      {
        filteredData.length > 15 && <Pagination
...
export default ToolsConsumbles;
